...
window.onload = function()
{
	...
	for(var i = 0; i <= 9; i++)
	{
		...
		if(i >= 1 && i <= 4)
		{
			...
			nom_div("operador_" + i).addEventListener('click', function(event)
			{
				...
				var operador = Number(this.id.split("_")[1]);
				console.log(operador);
				var txtOperador = operaciones[operador - 1].operador;					
				//console.log(txtOperador);
				if(!existeOperador(ultimo) && (operacion !== "" || txtOperador === "-"))
				{
			...
			if(i <= 3)
			{
				...
				{
					var accion = Number(this.id.split("_")[1]);
	...
	var insertaNumeros = function(numero)
	{
		...
		{
			//console.log(operacion.charAt(c), c);
	...
	var existeOperador = function(operador)
	{
		...
		for(var c = 0; c < operaciones.length; c++)
		{
